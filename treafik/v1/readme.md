...
HASHED_PASSWORD :  "$apr1$31r5t9Pq$gmSWbZ90kAieWBHC/6Tb0."
...
CLOUDFLARE_API_KEY
  value
0673c13f68323f58c04a2afd49944df19e4b1
...
services:
  apt_big_web:
    image: "bigbase/aptbigsrv:latest"    
    environment:
      #- ASPNETCORE_ENVIRONMENT=Docker
      #- ASPNETCORE_URLS http://+:5000
      - ASPNETCORE_URLS=https://+:443;http://+:80
      #- ASPNETCORE_URLS=http://+:80
      - ASPNETCORE_Kestrel__Certificates__Default__Password=whqotn#!70
      - ASPNETCORE_Kestrel__Certificates__Default__Path=/https/aspnetapp.pfx
    volumes:
      - /mnt/data/apt-db/.aspnet/https:/https:ro 
    #ports:
      #- "18078:80"
      #- "18079:443"
    networks:
    ...
    deploy:
      ...
      labels:
        ...
        #- traefik.enable=true
        #- traefik.constraint-label=cloud-public
        #- traefik.http.routers.aptbigweb.entrypoints=web
        #- traefik.http.routers.aptbigweb.rule=hostregexp(`{host:(www\.)?.+}`)
        #- traefik.http.routers.aptbigweb.middlewares=redirect-to-non-www-https

        #- traefik.http.routers.aptbigweb.entrypoints=websecure
        #- traefik.http.routers.aptbigweb.rule=Host(`www.dataleader.kr`) || Host(`dataleader.kr`)
        #- traefik.http.services.apt_big_web.loadbalancer.server.port=5000
        ...
        - traefik.http.routers.aptbigweb.entrypoints=websecure
        - traefik.http.routers.aptbigweb.rule=Host(`www.dataleader.kr`) || Host(`dataleader.kr`)
        - traefik.http.routers.aptbigweb.tls=true
        # min TLS version
        - traefik.http.routers.aptbigweb.tls.options=tls12@file
        - traefik.http.routers.aptbigweb.tls.certresolver=letsencrypt
        - traefik.http.routers.aptbigweb.service=apt_big_web
        - traefik.http.routers.aptbigweb.middlewares=traefik-ratelimit
        - traefik.http.middlewares.traefik-ratelimit.ratelimit.average=100
        - traefik.http.middlewares.traefik-ratelimit.ratelimit.burst=50
        - traefik.http.services.apt_big_web.loadbalancer.server.port=80
        
        #sticky 세션 지속
        - traefik.http.services.apt_big_web.loadbalancer.sticky=true
        - traefik.http.services.apt_big_web.loadbalancer.sticky.cookie.name=stickycookie
        - traefik.http.services.apt_big_web.loadbalancer.sticky.cookie.secure=true
        - traefik.http.services.apt_big_web.loadbalancer.sticky.cookie.httpOnly=false
